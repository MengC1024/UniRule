...
Defaults to ten repetitions of tenfold cross validation, as in Section 4.4.
...
def repeated_folds(rows, groups, folds, repetitions, seed):
    """Generate repeated grouped partitions or validate supplied partitions."""
    if any("folds" in row for row in rows):
        if (not all("folds" in row and len(row["folds"]) == repetitions for row in rows)
                or any("fold" in row for row in rows)):
            raise ValueError("Supply folds with one fold ID per repetition for every comparison")
        assignments = []
        for repeat in range(repetitions):
            partition = [{**row, "fold": row["folds"][repeat]} for row in rows]
            assignments.append(assign_folds(partition, groups, folds, seed)[0])
        return np.stack(assignments), "provided per repetition in comparison records"
    if any("fold" in row for row in rows) and repetitions != 1:
        raise ValueError("A single supplied fold partition requires --repetitions 1; "
                         "use folds lists to supply multiple partitions")
    assignments = [assign_folds(rows, groups, folds, seed + repeat)[0]
                   for repeat in range(repetitions)]
    source = ("provided in comparison records" if "fold" in rows[0]
              else "generated by original rule, stratified by language; seed + repetition index")
    return np.stack(assignments), source
...
def run_transfer(rows, *, methods, folds=10, bootstrap=1000, seed=20260914,
                 scheme="both", position=True, repetitions=10):
    if folds < 2 or bootstrap < 0 or repetitions < 1:
        raise ValueError("folds must be at least 2, repetitions positive, and bootstrap nonnegative")
    ...
    assignments, fold_source = repeated_folds(rows, groups, folds, repetitions, seed)
    # Share resampling weights across all repetitions and predictors. Repeated
    # partitions are not independent observations for confidence intervals.
    ...
    for repeat, fold in itertools.product(range(repetitions), range(folds)):
        assignment = assignments[repeat]
            ...
            try:
                ...
                for name in names:
                    ...
                    fold_details[name][target].append({
                        "repetition": repeat, "fold": fold,
                        "source_inputs": int(input_counts[train][:, mask].sum()),
                        "target_training_inputs": int(input_counts[train, target].sum()),
                        "target_test_inputs": int(input_counts[test, target].sum()),
                    })
            except ValueError as exc:
                raise ValueError(f"Repetition {repeat}, fold {fold}, target {language}/{form}: {exc}") from exc
        print(f"Completed repetition {repeat+1}/{repetitions}, fold {fold+1}/{folds}",
    ...
    result = {
        ...
        "original_rule_clusters": len(groups), "folds": folds, "repetitions": repetitions,
        "fold_source": fold_source,
        "seed": seed, "bootstrap_replicates": bootstrap, "position_adjusted": position,
        "uncertainty": "Paired original-rule bootstrap within language; the same resample is used across all fixed partitions and predictors, refitting every model in every repetition; pointwise 95% intervals",
        "averaging": "Average held-out squared errors across repetitions within each setting, then average target settings equally; predictions are not averaged before calculating errors",
        "difference_direction": "Positive bt_minus_target_mse means BT from sources has higher error",
        "fold_assignments": [{"language": g[0], "rule_id": g[1],
                              "folds": assignments[:, i].tolist()}
...
def main():
    ...
    try:
        result = run_transfer(load_comparisons(args.comparisons), methods=args.methods, folds=args.folds,
                              ...
                              position=not args.no_position, repetitions=args.repetitions)
